...
import { Modal as ModalAntd, Table as TableAntd } from "../../components/AntdComponent";
...
import { useWithdrawAmc } from "./useWithdrawHewe";
...
const WithdrawAMC = ({ history, setUsers, userData }) => {
  ...
  const getAdminList = async (
    ...
    try {
      ...
      const { data } = await axios.get(`/getAllSubAdmin?page=${page}&pageSize=${rowsPerPage}`, {
        headers: {
          Authorization: token,
        },
      });
        ...
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading style={{ display: "flex", flexDirection: "column" }}>
              ...
              >
                <i class="fa-solid fa-user-tie" style={{ fontSize: "25px", margin: "8px" }}></i>

                <DashHeading style={{ color: "#02001c", flex: "1", padding: "8px" }}>Withdraw AMC</DashHeading>
              </MenuAndBack>
            </DashboardHeading>
                  ...
                  <Form>
                    <div className="signup-cont">
                      <div className="row">
                        ...
                        <div className="col-md-6">
                          <label className={classes.offerLabel}>Admin Role</label>
                          <div className="mt-2">
                            <Select
                              options={options}
                              value={options.find((option) => option.value === formikBag.values.role)}
                              onChange={(selectedOption) => {
                                formikBag.setFieldValue("role", selectedOption.value);
                              }}
                        ...
                        <div className="col-md-6">
                          <label className={classes.offerLabel}>Access Modules</label>
                          <div className="mt-2">
                            <Select
                              ...
                              value={optionss.find((option) => option.value === formikBag.values.access_module)}
                              onChange={(selectedOptions) => {
                                const selectedValues = selectedOptions.map((option) => option.value);
                                formikBag.setFieldValue("access_module", selectedValues);
                              }}
                              classNamePrefix="react-select"
                              isClearable
                              isMulti
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(WithdrawAMC));
