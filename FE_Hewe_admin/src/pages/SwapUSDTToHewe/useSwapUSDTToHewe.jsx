...
const getListDataAPI = ({ limit, page, type, keyword, filterToken }) => {
  return axios.get(
    `/getSwapHistory?limit=${limit}&page=${page}&type=${type}&token=${filterToken}&keyword=${keyword}`,
    {
      headers: {
...
export const useSwapUSDTToHewe = ({ type }) => {
  const {
    x,
    y,
    currentPage,
  ...
  const handleGetListData = useCallback(
    ({ limit = LIMIT, page, keyword, filterToken }) => {
      handleGetData({
        paramsQuery: { limit, page, keyword, type, filterToken: filterToken },
      });
    },
    []
  );
  ...
  const columns = [
    ...
    },
    {
      title: "Amount Token",
      render: (_, record) => {
        if (record?.tokenBuy === "AMC")
          return round(record?.amountAmc) + " AMC";
        else if (record?.tokenBuy === "HEWE")
          return round(record?.amountHewe) + " HEWE";
        else return "";
      },
      width: 200,
    },
