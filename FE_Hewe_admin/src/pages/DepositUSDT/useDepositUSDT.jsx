...
const getListDataAPI = ({ limit, page, keyword, network }) => {
  return axios.get(
    `/getDepositUSDTHistory?limit=${limit}&page=${page}&keyword=${keyword}&network=${network}`,
    {
      headers: {
      ...
      render: (_, record) => {
        return (
          <div>
            ...
            {network === "BEP20" && record.logData.length > 10 && (
              <Tag color="green">Đã chuyển về ví admin</Tag>
            )}
          </div>
        );
      },
