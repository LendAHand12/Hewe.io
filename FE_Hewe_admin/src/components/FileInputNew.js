...
export default function FileInput(props) {
  ...
  return (
    <>
      <div className={`file_input_box_wrapper_new ${images.length >= limit && "disable"}`}>
        <div className="added_images_new">
          {images.map((item) => {
            return (
              <div className="img_box_new">
...

        {
          <div className="file_input_box">
            {/*<h3>{label} </h3>*/}
            <label htmlFor={id} accept={accept} className="drop_area_new">
              <img src={dropbutton} />
              <span>{dictionary.select_file}</span>
            </label>
            {/*<p>{info}</p>*/}
            <p className="warning">
              <span>{dictionary.please_note}</span> {warning || dictionary.please_note_text}
            </p>
          </div>
        }
